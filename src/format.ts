...
export function toSummary(report: CoverageReport) {
  ...
  return `${posture} ${report.programs} rollouts · ${report.packets} packets · ${report.highSeverityPackets} high-severity packets · ${report.workflowGaps} workflow gaps`;
}
...
export function toMarkdown(report: CoverageReport) {
  ...
  return [
    `# Employee AI policy posture ${report.ok ? "✅" : "❌"}`,
    "",
    `- Rollouts: **${report.programs}**`,
    `- On-track rollouts: **${report.onTrackPrograms}**`,
