...
function New-ProofImage {
    ...
    $graphics.DrawString("EMPLOYEE AI ACCEPTABLE USE BOARD", $eyebrowFont, $accentBrush, 92, 92)
...
New-ProofImage -Path (Join-Path $screenshots "01-overview-proof.png") `
    -Title "Employee AI policy review posture" `
    -Subtitle "Operator-readable proof for acceptable-use evidence, training readiness, exception routing, and rollout-safe governance." `
    -Bullets @(
        "Surface the same packets security, HR, and enablement teams argue over.",
        "Keep AI policy debt visible before it becomes a workforce trust or audit problem.",
        "Show recruiter-readable AI governance, acceptable use, and workforce policy depth."
    )

New-ProofImage -Path (Join-Path $screenshots "02-policy-lane-proof.png") `
    -Title "Lane ownership and policy routing" `
    -Subtitle "Each lane maps to a real employee AI policy operator responsibility instead of generic governance copy." `
    -Bullets @(
        "Policy evidence, training, exception, and manager lanes stay separated and accountable.",
        "Owner mapping keeps security awareness, identity governance, and HR enablement explicit.",
        "Findings roll up into rollout-safe next actions."
    )

New-ProofImage -Path (Join-Path $screenshots "03-rollout-posture-proof.png") `
    -Title "Rollout posture before workforce release" `
    -Subtitle "Decision packets show which AI policy rollouts can move and which should stop." `
    -Bullets @(
        "Completeness score, blocker, and review window stay in one view.",
        "High-severity packet debt is visible before employee AI launches go live.",
